...
usage() {
    ...
    echo
    echo "Target:"
    echo "    dist     Build RESTEasy binaries (default)."
    echo "    install  Install RESTEasy binaries."
...
generate_rpm_spec() {
    ...
    # hard-code patch
    if [ "$PATCH" != "" ] ; then
        commands="${commands}; s/# Patch: resteasy-VERSION-RELEASE.patch/Patch: $PATCH/g"
    fi
...
if [ "$BUILD_TARGET" = "dist" ] ; then
    ...
    echo "- Java archive: $WORK_DIR/resteasy.jar"
    echo
